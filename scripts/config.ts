...
const EnvSchema = z.object({
  ...
  INDEXER_MAX_RETRIES: z.coerce.number().int().nonnegative().default(DEFAULT_RETRY_OPTIONS.maxRetries),
});
...
export function loadIndexerConfig(env: NodeJS.ProcessEnv = process.env): IndexerConfig {
  ...
  const { REQUEST_DELAY_MS, MAX_CONCURRENT_REQUESTS, ARCHIVE_ORG_BASE_URL, INDEXER_MAX_RETRIES } = parsed.data;
  return {
