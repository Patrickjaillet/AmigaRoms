...
- `tests/app/`: a separate Vitest project (`vitest.workspace.ts`) running frontend tests under `jsdom` with `fake-indexeddb`, covering the cache and cached-loader modules; wired into `npm run typecheck` via `tests/app/tsconfig.json`.
